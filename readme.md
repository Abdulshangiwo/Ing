...
Pracownicy: Informacja o tym, który pracownik obsłużył dane zlecenie.

Schemat Bazy Danych
`Samochody` - informacje o pojazdach.
`Kategorie` - podział na klasy (np. SUV, Sedan, Ekonomiczne).
`Klienci` - dane osób wypożyczających.
`Pracownicy` - osoby odpowiedzialne za proces wydania auta.
`Wypozyczenia` - tabela łącząca, przechowująca historię transakcji.
